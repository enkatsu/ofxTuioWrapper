...
UdpSender::UdpSender() {
	try {
		local = true;
		long unsigned int ip = osc::GetHostByName("localhost");
        socket = new osc::UdpTransmitSocket(osc::IpEndpointName(ip, 3333));
...
UdpSender::UdpSender(const char *host, int port) {
	try {
        ...
        long unsigned int ip = osc::GetHostByName(host);
		socket = new osc::UdpTransmitSocket(osc::IpEndpointName(ip, port));
